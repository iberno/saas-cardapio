...
export class VariantesService {
  ...
  findAll(produtoId: string) {
    return this.prisma.scoped.produtoVariante.findMany({
      where: { produtoId },
      orderBy: { preco: 'asc' },
    });
  }
  ...
  async create(produtoId: string, dto: { nome: string; preco: number }) {
    const variante = await this.prisma.scoped.produtoVariante.create({
  ...
  async update(id: string, dto: { nome?: string; preco?: number }) {
    ...
    const variante = await this.prisma.scoped.produtoVariante.update({ where: { id }, data });
    await this.atualizarPrecoProduto(variante.produtoId);
    return variante;
  }

  async remove(id: string) {
    const v = await this.prisma.scoped.produtoVariante.findUnique({ where: { id } });
    if (!v) return;
    await this.prisma.scoped.produtoVariante.delete({ where: { id } });
    await this.atualizarPrecoProduto(v.produtoId);
  }

  private async atualizarPrecoProduto(produtoId: string) {
    const min = await this.prisma.scoped.produtoVariante.aggregate({
      where: { produtoId },
      _min: { preco: true },
    });
    await this.prisma.scoped.produto.update({
      where: { id: produtoId },
